...
This script will tell git to *by default* ignore prompt numbers and
cell output when looking at ipynb files. To include output for a specific
notebook, add the following to their metadata:

    "git" : { "suppress_output" : false }

The notebooks themselves are not changed.

This has been adapted from code in this blogpost:
...
1. Put this script in a directory that is on the system's path.
   ...
   `~/.config/git/drop_ipynb_output.py`.
2. Make sure it is executable by typing the command
   `chmod +x ~/.config/git/drop_ipynb_output.py`.
...
4. Connect this script to the filter by running the following
   ...
   git config --global filter.clean_ipynb.clean ~/.config/git/drop_ipynb_output.py
